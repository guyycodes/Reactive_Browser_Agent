...
- **Execute → Verify:** as of `week2b-skill-cards-runtime`, `executeStep` is YAML-driven via `executeSkillCardSteps` against skill cards in `kb/skill_cards/<app>/<skill>.yml`. `runExecuteStep` probes `ctx.browser` liveness at entry (see §2.1 "Real browser steps" bullet for the three-branch dispatch: live → reuse + resume at first destructive; dead → pre-close + fresh launch + full skill; no-prior-session → fresh launch + full skill). `verifyStep` runs a Sonnet `/verified/i` check. **Known false-positive gap flagged for Week-2 polish:** verifyStep can hallucinate `/verified/i` against `stepsRun=0` input; DB cross-check is mandatory acceptance for smoke runs (documented in MASTER_PLAN's commit-3-hotfix-1 meta-observation; reinforced by week2b-runtime bug-2 retro — see polish queue #2).
