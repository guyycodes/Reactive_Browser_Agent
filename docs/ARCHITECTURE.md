...
- **Pre-exec human gate (`review_gate`, Commits 2 + 7b.iii.b-pre-exec-refine + pre-exec-edit-ui):** suspends on `bus.awaitDecisionForStep(runId, "review_gate")` (per-stepId bus decisions, 7b.iii.b-bus-extension). **Three decision paths:**
  - `approve` → proceed to executeStep.
  - `reject` → terminate run as `status: rejected`.
  - `edit` with `patch.notes` → **refine loop:** synthesize `Pre-exec refine N: reviewer note: <note>` observation, emit `block.backtrack.triggered { fromStep: "review_gate", backtrackCount: N }`, re-run Block 1 with the note threaded through `runBlock1(..., { seedObservations: ... })` (Bug A Scope 2 fix, hotfix-2 — NO `withRunContext` spread here; see CTX SPREAD INVARIANT in `runContext.ts`), re-emit `review.requested`, re-await. Cap `MAX_PRE_GATE_REFINES = 2`. 3rd edit forces reject. Synthetic `block1 step.started/completed` frames wrap each refine's Block 1 call so the LEFT column re-renders the refined plan via `StepOutcome`'s `findLast` read path (Piece B of commit 4).
...
- **Post-exec human gate (`human_verify_gate`, Commit 4):** suspends on `bus.awaitDecisionForStep(runId, "human_verify_gate")`. **Two paths:**
  - `approve` (or `edit` — server treats edit ≡ approve, UI deliberately hides Edit to prevent a wedge state; see ReviewPanel docblock) → happy path; run terminates via logAndNotifyStep with `status: ok`.
  - `reject` → **backtrack loop:** `buildBacktrackContext(currentVerify, reviewerNote, backtrackCount)` produces a 4-5 entry observation array (header matching `/Backtrack \d+/` + reviewer note + stepsRun + success + evidence), emit `block.backtrack.triggered { fromStep: "human_verify_gate", backtrackCount: N }`, re-run the FULL pre-notify chain (block1 → reviewGate → execute → verify) with observations seeded. Cap `MAX_BACKTRACKS = 2`. 3rd reject forces `verify.success: false` + evidence marker; logAndNotifyStep terminates run with `status: failed`.
...
│  └────────────────────────────────────────────────────────────────────┘ │
│  ReviewPanel — natural flex flow, 4 renders driven by reviewHint +     │
│    mode state: [default: Edit/Reject/Approve] [edit: textarea+Submit]  │
│    [submitting: wait-for-plan + local-cancel] [post-exec: Reject+      │
│    Approve] [exhausted: Reject only] — see below                       │
└────────────────────────────────────────────────────────────────────────┘
```
...
- **Review panel (7b.iii.b series — three-mode + post-exec variant)** — remains outside `.review-layout` and sits in the natural flex flow at the bottom of `.review-page`. The `<ReviewPanel>` component is a state machine driven by `pendingReview.reviewHint` + internal `mode` state:

  - **Pre-exec (`reviewHint === "pre_exec"`, default when unset) — three modes:**
    - `default` — "Review requested" heading + `N actions · ⚠ destructive` body + three buttons (Edit / Reject / Approve).
    - `edit` (on Edit click) — "Guide the agent · re-plan" heading + textarea (2000-char cap matching `PlanPatchSchema.notes`, counter renders live, Submit disabled-until-non-empty) + Cancel / Submit & re-plan buttons.
    - `submitting` (on Submit) — "Re-planning…" heading + disabled "Waiting for refined plan…" button + local "Cancel refine (unlock panel)" escape hatch. Auto-exits to `default` via a `planId`-delta `useEffect` when the fresh `review.requested` arrives. The Cancel-refine button is a UI-only reset (does NOT cancel the server-side Block 1 refine — explicitly documented in the component's docblock); any Approve/Reject clicked on the stale request between local-cancel and fresh-request-arrival hits the bus's stale-discard path and is logged + dropped (commit 1 semantics).
  - **Exhausted (`blockResult.passedLast === false`)** — "⚠ Block 1 exhausted" banner with per-pass reasons as chips + Reject only (Approve disabled). Override-and-proceed is Week-2 polish.
  - **Post-exec (`reviewHint === "post_exec"`)** — "Post-execution review" heading + body referencing `execute:*` screenshots and verify output + **Reject (backtrack)** / **Approve (complete)** buttons only. **Edit is DELIBERATELY HIDDEN** on this path — `humanVerifyGateStep` treats `decision === "edit"` as equivalent to `approve` (both take the happy path), so exposing an Edit button that means the same thing as Approve would both confuse reviewers and wedge the `submitting`-mode state machine (which assumes edit triggers a fresh `review.requested`). Component docblock flags this explicitly so future contributors don't re-enable Edit without first changing the server semantic.
